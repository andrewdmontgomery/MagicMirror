...
describe('windDriftVector', () => {
  ...
  it('scales from the calm floor to the max', () => {
    ...
    assert.ok(Math.abs(mag(0) - 0.03) < 1e-9)
    assert.ok(Math.abs(mag(75) - 1.6) < 1e-9)
    assert.ok(Math.abs(mag(37.5) - 0.815) < 1e-9)
  })
...
describe('field sampling', () => {
  ...
  it('advects particles along the slotted field, not the fallback', () => {
    ...
    c.currentWindWindow = () => []
    withoutRespawn(() => def.advectParticles.call(c, ctx2d, 400, 400))
    // 10 m/s eastward = 22.37 mph: 0.03 + (22.37/75) * 1.57 px/frame.
    const step = (0.03 + ((10 * 2.23694) / 75) * 1.57) / 10
...
describe('wind slots', () => {
  ...
  it('dates the line by the displayed hour, across midnight', () => {
    ...
    const writes = []
    const c = ctx({
      timelineLabel: { set textContent (v) { writes.push(v) }, get textContent () { return writes[writes.length - 1] } },
      timelineTicks: { hasChildNodes: () => true },
      timelineTrack: { children: [] },
      windIndex: 0,
      view: 'wind',
...
describe('updateTimeline', () => {
  it('labels the latest frame Now and older frames by time', () => {
    ...
    const c = ctx({
      frames: framesFixture(5),
      frameIndex: 4,
      timelineLabel: { set textContent (v) { updated.push(v) }, get textContent () { return updated[updated.length - 1] } },
      timelineTicks: { hasChildNodes: () => true },
      timelineTrack: { children: [] }
    })
